...
def evaluateDepths(predDepths, gtDepths, printInfo=False):
    ...
    rmse_log = np.sqrt(
        (
            pow(
                np.log(np.maximum(predDepths, 1e-4))
                - np.log(np.maximum(gtDepths, 1e-4)),
                2,
            )
            * masks
        ).sum()
    ...
    log10 = (
        np.abs(
            np.log10(np.maximum(predDepths, 1e-4))
            - np.log10(np.maximum(gtDepths, 1e-4))
        )
        * masks
    ).sum() / numPixels
    rel = (
        np.abs(predDepths - gtDepths) / np.maximum(gtDepths, 1e-4) * masks
    ).sum() / numPixels
    rel_sqr = (
        pow(predDepths - gtDepths, 2) / np.maximum(gtDepths, 1e-4) * masks
    ).sum() / numPixels
    deltas = (
        np.maximum(
            predDepths / np.maximum(gtDepths, 1e-4),
...
def evaluatePlanesTensor(input_dict, detection_dict, printInfo=False, use_gpu=True):
    ...
    if use_gpu:
        masks_pred, masks_gt, depth_pred, depth_gt = (
            detection_dict["masks"],
            input_dict["masks"],
            detection_dict["depth"],
            input_dict["depth"],
        )
    else:
        masks_pred, masks_gt, depth_pred, depth_gt = (
            detection_dict["masks"].cpu(),
            input_dict["masks"].cpu(),
            detection_dict["depth"].cpu(),
            input_dict["depth"].cpu(),
        )
        pass
    ...
    depths_diff = (depth_diff * masks_intersection).sum(2).sum(2) / torch.clamp(
        intersection_areas, min=1e-4
    )
    depths_diff[intersection_areas < 1e-4] = 1000000

    union = (
        ((masks_gt.unsqueeze(1) + masks_pred.unsqueeze(0)) > 0.5).float().sum(2).sum(2)
    )
    ...
    for IOU_threshold in [
        ...
        for step in range(21):
            ...
            pixel_recall.append(
                np.minimum(
                    (
                        intersection_areas
                        * (
                            (depths_diff <= diff_threshold).astype(np.float32)
                            * IOU_mask
                        )
                    ).sum(1),
                    plane_areas,
            ...
            plane_recall.append(
                float((min_diff <= diff_threshold).sum()) / len(masks_gt)
            )
            continue
    ...
    if printInfo:
        print(
            "plane statistics", correct_mask.max(-1).sum(), num_targets, num_predictions
        )
        pass
...
def evaluatePlaneDepth(config, input_dict, detection_dict, printInfo=False):
    ...
    plane_diff = torch.norm(
        plane_parameters_array[0] - plane_parameters_array[1], dim=-1
    )
    plane_areas = masks_gt.sum(-1).sum(-1)

    statistics = [
        plane_diff.mean(),
        (plane_diff * plane_areas).sum() / plane_areas.sum(),
    ]
    if printInfo:
...
def evaluateMask(predMasks, gtMasks, printInfo=False):
    ...
    info = [
        intersection / max(predMasks.sum(), 1),
        intersection / max(gtMasks.sum(), 1),
    ]
    if printInfo:
...
def evaluateMasksTensor(predMasks, gtMasks, valid_mask, printInfo=False):
    gtMasks = torch.cat(
        [gtMasks, torch.clamp(1 - gtMasks.sum(0, keepdim=True), min=0)], dim=0
    )
    predMasks = torch.cat(
        [predMasks, torch.clamp(1 - predMasks.sum(0, keepdim=True), min=0)], dim=0
    )
    intersection = (
        (gtMasks.unsqueeze(1) * predMasks * valid_mask).sum(-1).sum(-1).float()
    )
    union = (
        (torch.max(gtMasks.unsqueeze(1), predMasks) * valid_mask)
        .sum(-1)
        .sum(-1)
        .float()
    )
    ...
    log2_quotient = (
        torch.log2(torch.clamp(joint, 1e-8) / torch.clamp(B, 1e-8))
        * (torch.min(joint, B) > 1e-8).float()
    )
    ...
    SC = (
        (
            IOU.max(-1)[0]
            * torch.clamp((gtMasks * valid_mask).sum(-1).sum(-1), min=1e-4)
        ).sum()
        / N
        + (
            IOU.max(0)[0]
            * torch.clamp((predMasks * valid_mask).sum(-1).sum(-1), min=1e-4)
        ).sum()
        / N
    ) / 2
    info = [RI, voi, SC]
...
def evaluateBatchDeMoN(options, gt_dict, pred_dict, statistics=[[], [], []]):
    for batchIndex in range(len(gt_dict["depth"])):
        statistics[0].append(
            evaluateDepthRelative(
                pred_dict["depth"][batchIndex],
                gt_dict["depth"][batchIndex],
                np.linalg.norm(gt_dict["translation"][batchIndex]),
            )
        )
        statistics[1].append(
            evaluateRotation(
                pred_dict["rotation"][batchIndex], gt_dict["rotation"][batchIndex]
            )
        )
        statistics[2].append(
            evaluateTranslation(
                pred_dict["translation"][batchIndex], gt_dict["translation"][batchIndex]
            )
        )
        continue
    return
...
def evaluateBatchDetection(
    ...
    debug_dict={},
    printInfo=False,
    evaluate_plane=False,
):
    if "depth" in debug_dict:
        planes = fitPlanesModule(
            config, debug_dict["depth"][80:560], detection_dict["masks"][:, 80:560]
        )
        detections = detection_dict["detection"]
        detections = torch.cat([detections[:, :6], planes], dim=-1)
        depth, detection_mask = calcDepthModule(
            config, detections, detection_dict["masks"]
        )
    ...
    for c, plane_mask in enumerate([nyu_mask]):
        ...
        if padding > 0:
            valid_mask_depth = valid_mask_depth[
                :, 80 + padding : 560 - padding, padding:-padding
            ]
        else:
        ...
        if options.debug:
            print("\nmask", c)
            pass
        depth_statistics = evaluateDepths(
            depth_pred[valid_mask_depth].detach().cpu().numpy(),
            depth_gt[valid_mask_depth].detach().cpu().numpy(),
            printInfo=printInfo,
        )
    ...
    statistics[2].append(
        evaluateMasksTensor(
            torch.round(detection_dict["masks"]).cpu(),
            input_dict["masks"].float().cpu(),
            valid_mask.float().cpu(),
            printInfo=printInfo,
        )
    )

    if "masks" in detection_dict and evaluate_plane:
        plane_statistics = evaluatePlanesTensor(
            input_dict, detection_dict, printInfo=printInfo
        )
...
def printStatisticsDetection(options, statistics):
    ...
    with open("logs/global.txt", open_type) as f:
        values = (
            np.array(statistics[0]).mean(0).tolist()
            + np.array(statistics[1]).mean(0).tolist()
            + np.array(statistics[2]).mean(0).tolist()
        )
...
def plotCurves(
    filename="test/curves.png",
    xlabel="depth threshold",
    ylabel="per plane recall %",
    title="",
    methods=[
        "manhattan_pred",
        "manhattan_gt",
        "planenet_normal",
        "refine_normal_refine",
    ],
):
    ...
    for order in ordering:
        plt.plot(
            ...
            ys[order],
            figure=fig,
            label=final_labels[order],
            color=colors[order],
            marker=markers[order],
            linewidth=sizes[order],
        )
        continue
    plt.legend(
        loc="upper right",
...
def writeTable(
    filename="logs/table.txt",
    methods={
        "planenet_normal": "PlaneNet",
        "warping_normal_pair": "Ours",
        "basic_normal_backup": "Ours (w/o warping loss)",
        "warping_normal_none_pair": "Ours (w/o normal anchors",
        "warping_joint_pair": "Ours (w/o depth map)",
    },
    cols=[20, 19, 21, 32, 38, 44],
    dataset="",
):
