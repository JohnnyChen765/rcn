...
class InferenceDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        segmentation = np.zeros(
            (self.config.IMAGE_MIN_DIM, self.config.IMAGE_MAX_DIM), dtype=np.int32
        )
        ...
        if len(planes) > 0:
            if "joint" in self.config.ANCHOR_TYPE:
                distances = np.linalg.norm(
                    np.expand_dims(planes, 1) - self.config.ANCHOR_PLANES, axis=-1
                )
                ...
                distances_d = np.abs(
                    np.expand_dims(plane_offsets, -1) - self.config.ANCHOR_OFFSETS
                )
        ...
        for planeIndex, plane in enumerate(planes):
            ...
            if self.config.ANCHOR_TYPE == "none":
                ...
                parameters.append(
                    np.concatenate(
                        [residual, np.array([0, plane_info[planeIndex][-1]])], axis=0
                    )
                )
            elif self.config.ANCHOR_TYPE == "Nd":
                class_ids.append(
                    normal_anchors[planeIndex] * len(self.config.ANCHOR_OFFSETS)
                    + offset_anchors[planeIndex]
                    + 1
                )
                normal = (
                    plane_normals[planeIndex]
                    - self.config.ANCHOR_NORMALS[normal_anchors[planeIndex]]
                )
                offset = (
                    plane_offsets[planeIndex]
                    - self.config.ANCHOR_OFFSETS[offset_anchors[planeIndex]]
                )
        ...
        (
            ...
            self.config, index, image, depth, mask, class_ids, parameters, augment=False
        )
        ## RPN Targets
        ...
        depth = np.concatenate(
            [np.zeros((80, 640)), depth, np.zeros((80, 640))], axis=0
        ).astype(np.float32)
        segmentation = np.concatenate(
            [
                np.full((80, 640), fill_value=-1),
                segmentation,
                np.full((80, 640), fill_value=-1),
            ],
            axis=0,
